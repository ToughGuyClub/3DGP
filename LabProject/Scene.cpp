...
void CScene::BuildObjects()
{
	...
	CCubeMesh *pObjectCubeMesh = (CCubeMesh*)new CModelMesh("model/enemy.obj");
...
void CScene::CheckPlayerByEnemyBulletCollisions()
{
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		CExplosiveObject* pEnemy = dynamic_cast<CExplosiveObject*>(m_ppObjects[i]);
		if (!pEnemy) continue;

		
		for (int j = 0; j < ENEMY_BULLETS; j++)
		{
			CBulletObject* pBullet = pEnemy->mm_ppBullets[j];

			if (pBullet->m_bActive && m_pPlayer->m_xmOOBB.Intersects(pBullet->m_xmOOBB))
			{
				
				pEnemy->mm_ppBullets[j]->m_bActive = false;
				m_pPlayer->m_playerHP -= 1;
